...
Com_Quit(void)
{
	...
	{
		/* Exit Game can arrive while a local gameplay server is active. The
		 * physically confirmed menu-first workaround succeeds after that server
		 * is already gone, while the direct path can freeze before Sys_Quit gets
		 * to its guarded renderer teardown. Do not run save/game-module cleanup
		 * after XMB has committed to terminating the process; LV2 reclaims those
		 * process-owned resources. A normal console quit still shuts it down. */
		PS3_RUNTIME_TRACE("Com_Quit: XMB exit skips active server teardown");
	}
	else
#endif
	{
		PS3_RUNTIME_TRACE("Com_Quit: before server shutdown");
		SV_Shutdown("Server quit\n", false);
		PS3_RUNTIME_TRACE("Com_Quit: server shutdown complete");
	}
	Sys_Quit();
}
