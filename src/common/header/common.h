...
#ifdef __PSL1GHT__
void Sys_BootTrace(const char *stage, qboolean reset);
qboolean Sys_PS3_CheckCallbacks(void);
qboolean Sys_PS3_ExitRequested(void);
qboolean Sys_PS3_OSKOpen(const char *prompt, const char *initial_text,
	int max_length, qboolean numbers_only, qboolean address_input);
qboolean Sys_PS3_OSKActive(void);
qboolean Sys_PS3_OSKPoll(char *text, size_t text_size, qboolean *accepted);
/* The original bring-up trace had hundreds of call sites. Keeping all of
 * them pushes the SELF's first executable segment past the range handled
 * reliably by the PS3/RPCS3 import loader. Leave those diagnostics compiled
 * out and opt important runtime/lifecycle events in explicitly. */
#define PS3_BOOT_TRACE(stage) ((void)0)
#define PS3_RUNTIME_TRACE(stage) Sys_BootTrace((stage), false)
#else
#define PS3_BOOT_TRACE(stage) ((void)0)
#define PS3_RUNTIME_TRACE(stage) ((void)0)
#endif
