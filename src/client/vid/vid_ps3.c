 ...
 *     Becasue there is no such thing as shared libraries on PS3 parts of
 *     engine should be compiled together. So I made precompile directives
 *     which are controled by UNICORE define, w/o it those files are
 *     identical to their origins. And they could be used to compile
 *     self contained game executable for any platform (base game,
 *     software renderer).
 * 
 * -----------------------------------------------------------------------
 */
...
VID_ShutdownRenderer(void)
{
	if (ref_active)
	{
		...
		{
			/* Once XMB has stopped display flips, the normal renderer teardown
			 * can block in RSX completion. The process owns these resources and
			 * LV2 will reclaim them after the remaining client shutdown. */
			PS3_RUNTIME_TRACE("VID_ShutdownRenderer: XMB exit skips blocking renderer teardown");
...
#ifdef UNICORE
extern refexport_t GetRefAPI(refimport_t imp);
#endif
/*
...
VID_LoadRenderer(void)
{
	...
	snprintf(reflib_name, sizeof(reflib_name), "built_in_rsx");
	#else
	snprintf(reflib_name, sizeof(reflib_name), "built_in_soft");
	#endif
#else
...
VID_Init(void)
{
	PS3_BOOT_TRACE("VID_Init: entered");
	...
	PS3_BOOT_TRACE("VID_Init: before GLimp_Init");
	if (!GLimp_Init())
	{
		Com_Error(ERR_FATAL, "Couldn't initialize the graphics subsystem!\n");
	}
	PS3_BOOT_TRACE("VID_Init: GLimp_Init complete");

	// Load the renderer and get things going.
	PS3_BOOT_TRACE("VID_Init: before VID_CheckChanges");
	VID_CheckChanges();
	PS3_BOOT_TRACE("VID_Init: complete");
}
...
VID_Shutdown(void)
{
	PS3_RUNTIME_TRACE("VID_Shutdown: entered");
	VID_ShutdownRenderer();
	GLimp_Shutdown();
	PS3_RUNTIME_TRACE("VID_Shutdown: complete");
}
...
void
Draw_GlyphStringScaled(int x, int y, const char *text, int count,
		float scale, int xor_mask)
{
	int i;

	if (!ref_active || !text || count <= 0)
	{
		return;
	}

	/* Console storage uses fixed-width, space-filled rows. Transparent trailing
	 * cells do not affect placement or erase pixels, so do not make the native
	 * renderer inspect them twice (visibility and command emission) every frame.
	 * The low seven bits identify a space in both normal and high-bit fonts. */
	while (count > 0 && (((unsigned char)text[count - 1] & 127) == 32))
	{
		count--;
	}
	if (count <= 0)
	...
	if (re.DrawGlyphStringScaled)
	{
		re.DrawGlyphStringScaled(x, y, text, count, scale, xor_mask);
		return;
	}

	for (i = 0; i < count; i++)
	{
		re.DrawCharScaled(x + (int)(i * 8 * scale), y,
