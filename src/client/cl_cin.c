...
/*
 * A Quake II CIN frame can contain up to 128 KiB of compressed video plus
 * about 6 KiB of audio. Keeping both buffers on the main thread stack is
 * harmless on desktop platforms, but can overflow the much smaller GameOS
 * PPU stack before the first cinematic frame is displayed. The decoder is
 * single-threaded, so fixed work buffers are safe here.
 */
YQ2_ALIGNAS_TYPE(short) static byte cinematic_samples[22050 / 14 * 4];
static byte cinematic_compressed[0x20000];
...
SCR_ReadNextFrame(void)
{
	...
	if (((size_t)size > sizeof(cinematic_compressed)) || (size < 1))
	{
		Com_Error(ERR_DROP, "Bad compressed frame size");
	}

	FS_Read(cinematic_compressed, size, cl.cinematic_file);
	...
	FS_Read(cinematic_samples, count * cin.s_width * cin.s_channels,
			cl.cinematic_file);
	...
	if (cin.s_width == 2)
	{
		...
		{
			((short *)cinematic_samples)[r] =
				LittleShort(((short *)cinematic_samples)[r]);
		}
	}

	S_RawSamples(count, cin.s_rate, cin.s_width, cin.s_channels,
			cinematic_samples, Cvar_VariableValue("s_volume"));

	in.data = cinematic_compressed;
	in.count = size;
