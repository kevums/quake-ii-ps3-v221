...
{
	Draw_GlyphStringScaled(x, y, s, (int)strlen(s), factor, 0);
}

void
DrawAltStringScaled(int x, int y, char *s, float factor)
{
	Draw_GlyphStringScaled(x, y, s, (int)strlen(s), factor, 0x80);
}
...
Con_DrawInput(void)
{
	...
	Draw_GlyphStringScaled(8 * scale, con.vislines - 22 * scale, text,
		con.linewidth, scale, 0);
...
Con_DrawNotify(void)
{
	...
	for (i = con.current - NUM_CON_TIMES + 1; i <= con.current; i++)
	{
		...
		Draw_GlyphStringScaled(8 * scale, v * scale, text,
			con.linewidth, scale, 0);
	...
	if (cls.key_dest == key_message)
	{
		...
		x = (int)strlen(s);
		Draw_GlyphStringScaled((skip << 3) * scale, v * scale, s, x,
			scale, 0);
...
Con_DrawConsole(float frac)
{
	...
	Draw_GlyphStringScaled(viddef.width - ((verLen * 8 + 5) * scale),
		lines - 35 * scale, version, verLen, scale, 0x80);
	...
	Draw_GlyphStringScaled(viddef.width - (173 * scale), lines - 25 * scale,
		tmpbuf, 21, scale, 0x80);

	/* draw the text */
	con.vislines = lines;

	/* Only walk rows that can reach the scaled viewport. The inherited count
	 * was based on unscaled eight-pixel rows, so at 2x/3x it rebuilt dozens of
	 * fully hidden console strings every frame. */
	rows = (int)((lines - 22.0f * scale) / (8.0f * scale));
	if (rows < 0)
	{
		rows = 0;
	}
	...
	for (i = 0; i < rows; i++, y -= 8, row--)
	{
		...
		Draw_GlyphStringScaled(8 * scale, y * scale, text,
			con.linewidth, scale, 0);
	}
...
#endif
	{
		...
		Draw_GlyphStringScaled(8 * scale, y * scale, dlbar,
			(int)strlen(dlbar), scale, 0);
	}
