...
#ifdef __PSL1GHT__
#include "input/header/psmove_ps3.h"
#endif

#ifdef PS3_NATIVE_RSX
extern void RSXGL_ApplyDisplayFilter(void);
#endif
...
SCR_DrawPSMoveStatus(void)
{
	...
	if (!PSMove_ShouldDrawStatus())
	{
		return;
	}

	title = PSMove_GetStatusText();
	detail = PSMove_GetStatusDetail();
	scale = SCR_GetMenuScale();
	title_width = (int)strlen(title) * 8;
	detail_width = (int)strlen(detail) * 8;
	width = title_width > detail_width ? title_width : detail_width;
	x = (int)((viddef.width / scale - width) * 0.5f);
	y = 12;

	Draw_Fill((int)((x - 8) * scale), (int)((y - 6) * scale),
		(int)((width + 16) * scale), (int)(30 * scale), 0);
	Draw_GlyphStringScaled((int)(x * scale), (int)(y * scale), title,
		(int)strlen(title), scale, 0x80);
	Draw_GlyphStringScaled((int)(x * scale), (int)((y + 11) * scale), detail,
		(int)strlen(detail), scale, 0);
}
#endif
...
SCR_DrawCenterString(void)
{
	...
	do
	{
		...
		Draw_GlyphStringScaled(x * scale, y * scale, start, l, scale, 0);
		x += l * char_unscaled_width;
...
SCR_Init(void)
{
	...
	cvar_t *menu_scale_revision;
	cvar_t *entity_shadows;
	cvar_t *shadow_revision;
	#endif
	...
	#ifdef __PSL1GHT__
	/* Yamagi's automatic integer scale enlarges the 8x8 console charset at HD
	 * resolutions. On PS3 that makes dense console output resemble overprinted
	 * or duplicated text. v1.33 migrated only an inherited automatic value, but
	 * installs which had already consumed that revision could later carry -1 or
	 * a large explicit value across update-only packages. Normalize once more so
	 * v2.06 starts every existing install at the physically legible PS3 1x
	 * baseline. The revision keeps every later user selection authoritative. */
	console_scale_revision = Cvar_Get("ps3_console_scale_revision", "0",
		CVAR_ARCHIVE);
	if (console_scale_revision->value < 2.0f)
	{
		Cvar_Set("r_consolescale", "1");
		Cvar_Set("ps3_console_scale_revision", "2");
	}
	/* v1.34 treated the persistent green/white rows as font scaling and migrated
	 * automatic menus to 1x. Physical testing proved they are stale framebuffer
	 * contents instead. Undo only that known v1.34 migration; keep pre-v1.34 and
	 * explicit user scale choices intact. */
	menu_scale_revision = Cvar_Get("ps3_menu_scale_revision", "0",
		CVAR_ARCHIVE);
	if (menu_scale_revision->value < 2.0f)
	{
		if (menu_scale_revision->value >= 1.0f &&
			r_menuscale->value == 1.0f)
		{
			Cvar_Set("r_menuscale", "-1");
		}
		Cvar_Set("ps3_menu_scale_revision", "2");
	}
	/* The desktop default disables Quake II's projected entity shadows. Enable
	 * them once for PS3 installs; the Video Options toggle and every subsequent
	 * explicit user choice remain authoritative after this revision. */
	entity_shadows = Cvar_Get("r_shadows", "0", CVAR_ARCHIVE);
	shadow_revision = Cvar_Get("ps3_shadow_revision", "0", CVAR_ARCHIVE);
	if (shadow_revision->value < 1.0f)
	{
		if (entity_shadows->value == 0.0f)
		{
			Cvar_Set("r_shadows", "1");
		}
		Cvar_Set("ps3_shadow_revision", "1");
	}
	#endif
...
DrawHUDStringScaled(char *string, int x, int y, int centerwidth, int xor, float factor)
{
	...
	while (*string)
	{
		...
		Draw_GlyphStringScaled(x, y, line, width, factor, xor);
...
SCR_UpdateScreen(void)
{
	...
	float scale = SCR_GetMenuScale();
	#ifdef PS3_NATIVE_RSX
	static qboolean traced_first_native_screen;
	if (!traced_first_native_screen)
	{
		PS3_BOOT_TRACE("Native first screen: entered");
	}
	#endif
...
#ifdef __PSL1GHT__
	if (Cvar_VariableValue("ps3_stereo_enable") > 0.0f)
#else
	if (gl1_stereo->value)
#endif
	{
...
#ifdef __PSL1GHT__
		{
			float depth = Cvar_VariableValue("ps3_stereo_depth");
			...
			separation[0] = -gl1_stereo_separation->value * depth / 2.0f;
			separation[1] = +gl1_stereo_separation->value * depth / 2.0f;
		}
#else
	...
	for (i = 0; i < numframes; i++)
	{
		...
		if (!traced_first_native_screen)
		{
			PS3_BOOT_TRACE("Native first screen: BeginFrame complete");
		}
		#endif
		...
		else
		{
			...
			if (!traced_first_native_screen)
			{
				PS3_BOOT_TRACE("Native first screen: before tile clear");
			}
			#endif
			SCR_TileClear();
			#ifdef PS3_NATIVE_RSX
			if (!traced_first_native_screen)
			{
				PS3_BOOT_TRACE("Native first screen: tile clear complete");
				PS3_BOOT_TRACE("Native first screen: before view render");
			}
			#endif
			...
			if (!traced_first_native_screen)
			{
				PS3_BOOT_TRACE("Native first screen: view render complete");
			}
			#endif
			...
			if (!traced_first_native_screen)
			{
				PS3_BOOT_TRACE("Native first screen: before console draw");
			}
			#endif
			SCR_DrawConsole();
			#ifdef PS3_NATIVE_RSX
			if (!traced_first_native_screen)
			{
				PS3_BOOT_TRACE("Native first screen: console draw complete");
				PS3_BOOT_TRACE("Native first screen: before menu draw");
			}
			#endif
			...
			if (!traced_first_native_screen)
			{
				PS3_BOOT_TRACE("Native first screen: menu draw complete");
			}
			#endif

			#ifdef __PSL1GHT__
			SCR_DrawPSMoveStatus();
			#endif
			SCR_DrawLoading();
		}

		#ifdef PS3_NATIVE_RSX
		/* Presentation effects must close each eye independently. Applying once
		 * after this loop would leave the first frame-packed eye unfiltered. */
		RSXGL_ApplyDisplayFilter();
		#endif
	}
	...
	if (!traced_first_native_screen)
	{
		PS3_BOOT_TRACE("Native first screen: before EndFrame");
	}
	#endif
	R_EndFrame();
	#ifdef PS3_NATIVE_RSX
	if (!traced_first_native_screen)
	{
		PS3_BOOT_TRACE("Native first screen: EndFrame complete");
		traced_first_native_screen = true;
	}
	#endif
}
