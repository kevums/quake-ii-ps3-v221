...
#ifdef __PSL1GHT__
static menufield_s *ps3_osk_field;

static void
Field_PollPS3OSK(void)
{
	char result[sizeof(((menufield_s *)0)->buffer)];
	qboolean accepted = false;
	menufield_s *field;

	if (!ps3_osk_field || !Sys_PS3_OSKPoll(result, sizeof(result), &accepted))
	{
		return;
	}
	field = ps3_osk_field;
	...
	Q_strlcpy(field->buffer, result, sizeof(field->buffer));
	if (field->length >= 0 && field->length < (int)sizeof(field->buffer))
	{
		field->buffer[field->length] = '\0';
	}
	field->cursor = (int)strlen(field->buffer);
	field->visible_offset = field->cursor > field->visible_length ?
		field->cursor - field->visible_length : 0;
	if (field->generic.callback)
	{
...
{
#ifdef __PSL1GHT__
	if (!ps3_osk_field && !Sys_PS3_OSKActive())
	{
		char prompt[96];
		const char *field_name = f->generic.name;
		if (!field_name || !field_name[0])
		{
			field_name = (f->generic.flags & QMF_OSK_ADDRESS) ?
				"server address" : "text";
		}
		snprintf(prompt, sizeof(prompt), "Enter %s", field_name);
		if (Sys_PS3_OSKOpen(prompt, f->buffer, f->length,
			(f->generic.flags & QMF_NUMBERSONLY) != 0,
			(f->generic.flags & QMF_OSK_ADDRESS) != 0))
		{
			ps3_osk_field = f;
			Key_MarkAllUp();
			return true;
...
Menu_AddItem(menuframework_s *menu, void *item)
{
	...
	if (menu->nitems < MAXMENUITEMS)
	{
		...
		#ifdef __PSL1GHT__
		if (((menucommon_s *)item)->type == MTYPE_FIELD &&
			!((menucommon_s *)item)->statusbar)
		{
			((menucommon_s *)item)->statusbar =
				"press Cross to open the PS3 on-screen keyboard";
		}
		#endif
		menu->nitems++;
	}
...
Menu_DrawString(int x, int y, const char *string)
{
	...
	Draw_GlyphStringScaled(x, y * scale, string, (int)strlen(string),
		scale, 0);
}
...
Menu_DrawStringR2L(int x, int y, const char *string)
{
	...
	Draw_GlyphStringScaled(x - (length - 1) * 8 * scale, y * scale,
		string, length, scale, 0);
}
