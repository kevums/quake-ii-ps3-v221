...
M_Print(int x, int y, char *str)
{
	const char *line;
	float scale = SCR_GetMenuScale();
	const int centered_x = (int)(viddef.width - 320 * scale) >> 1;
	const int centered_y = (int)(viddef.height - 240 * scale) >> 1;

	/* This is the original Quake II green/white menu printer. Keep each line
	 * as one atomic glyph run just like the console, HUD, inventory, and qmenu
	 * paths. The old per-character calls repeatedly ended and restarted the
	 * inline RSX command sequence, leaving this as the last substantial text
	 * route that was not published as a complete ordered line. */
	line = str;
	while (line && *line)
	{
		...
		if (length > 0)
		{
			Draw_GlyphStringScaled((int)(x * scale) + centered_x,
				(int)(y * scale) + centered_y, line, length, scale, 0x80);
		}
		if (!end)
		{
			break;
...
static const char *
ControllerButtonDisplayName(int key)
{
#ifdef __PSL1GHT__
	switch (key)
	{
		case K_BTN_A: return "CROSS";
		case K_BTN_B: return "CIRCLE";
		case K_BTN_X: return "SQUARE";
		case K_BTN_Y: return "TRIANGLE";
		case K_BTN_BACK: return "SELECT";
		case K_BTN_START: return "START";
		case K_STICK_LEFT: return "L3";
		case K_STICK_RIGHT: return "R3";
		case K_SHOULDER_LEFT: return "L1";
		case K_SHOULDER_RIGHT: return "R1";
		case K_TRIG_LEFT: return "L2";
		case K_TRIG_RIGHT: return "R2";
		case K_DPAD_UP: return "DPAD UP";
		case K_DPAD_DOWN: return "DPAD DOWN";
		case K_DPAD_LEFT: return "DPAD LEFT";
		case K_DPAD_RIGHT: return "DPAD RIGHT";
		default: break;
	}
#endif
...
DrawControllerButtonBindingFunc(void *self)
{
	...
	else
	{
		...
		name = ControllerButtonDisplayName(keys[0]);
		...
		if (keys[1] != -1)
		{
			...
			Menu_DrawString(a->generic.x + a->generic.parent->x + 48 * scale + (x * scale),
					a->generic.y + a->generic.parent->y,
					ControllerButtonDisplayName(keys[1]));
		}
	}
}
...
ControllerButtons_MenuInit(void)
{
	...
	#ifdef __PSL1GHT__
	Menu_SetStatusBar(&s_controller_buttons_menu,
		"CROSS assigns, TRIANGLE clears, CIRCLE exits");
	#else
	Menu_SetStatusBar(&s_controller_buttons_menu,
		"BTN_A assigns, BTN_Y clears, BTN_B exits");
	#endif
...
#ifdef __PSL1GHT__
static menulist_s s_psmove_enable_list;
static menuslider_s s_psmove_yawsensitivity_slider;
static menuslider_s s_psmove_pitchsensitivity_slider;
static menuslider_s s_psmove_deadzone_slider;
static menuslider_s s_psmove_smoothing_slider;
static menulist_s s_psmove_invertpitch_list;
#ifndef PS3_V198_OSK_RECOVERY
static menuaction_s s_psmove_calibrate_action;
#endif
static menuaction_s s_psmove_reinit_action;
#endif
...
#ifdef __PSL1GHT__
static void
PSMoveEnableFunc(void *unused)
{
    Cvar_SetValue("psmove_enable", s_psmove_enable_list.curvalue);
    Cbuf_AddText("psmove_reinit\n");
}

static void
PSMoveYawSensitivityFunc(void *unused)
{
    Cvar_SetValue("psmove_yawsensitivity",
        s_psmove_yawsensitivity_slider.curvalue / 10.0F);
}

static void
PSMovePitchSensitivityFunc(void *unused)
{
    Cvar_SetValue("psmove_pitchsensitivity",
        s_psmove_pitchsensitivity_slider.curvalue / 10.0F);
}

static void
PSMoveDeadzoneFunc(void *unused)
{
    Cvar_SetValue("psmove_deadzone",
        s_psmove_deadzone_slider.curvalue / 100.0F);
}

static void
PSMoveSmoothingFunc(void *unused)
{
    Cvar_SetValue("psmove_smoothing",
        s_psmove_smoothing_slider.curvalue / 10.0F);
}

static void
PSMoveInvertPitchFunc(void *unused)
{
    Cvar_SetValue("psmove_invertpitch", s_psmove_invertpitch_list.curvalue);
}

#ifndef PS3_V198_OSK_RECOVERY
static void
PSMoveCalibrateFunc(void *unused)
{
    Cbuf_AddText("psmove_calibrate\n");
}
#endif

static void
PSMoveReinitFunc(void *unused)
{
    Cbuf_AddText("psmove_reinit\n");
}
#endif
...
Joy_MenuInit(void)
{
    ...
    int y = 0;
#ifdef __PSL1GHT__
    static const char *offon_names[] = {"off", "on", 0};
    static const char *no_names[] = {"no", "yes", 0};
#endif
    ...
    s_psmove_enable_list.generic.type = MTYPE_SPINCONTROL;
    s_psmove_enable_list.generic.x = 0;
    s_psmove_enable_list.generic.y = y;
    y += 10;
    s_psmove_enable_list.generic.name = "PS Move aiming";
    s_psmove_enable_list.generic.callback = PSMoveEnableFunc;
#ifdef PS3_V198_OSK_RECOVERY
    s_psmove_enable_list.generic.statusbar =
        "gyro aiming; no PlayStation Eye required";
#else
    s_psmove_enable_list.generic.statusbar =
        "explicitly starts Move after boot; Navigation remains a normal gamepad";
#endif
    s_psmove_enable_list.itemnames = offon_names;
    s_psmove_enable_list.curvalue =
        (Cvar_VariableValue("psmove_enable") != 0.0F);
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_enable_list);

    s_psmove_yawsensitivity_slider.generic.type = MTYPE_SLIDER;
    s_psmove_yawsensitivity_slider.generic.x = 0;
    s_psmove_yawsensitivity_slider.generic.y = y;
    y += 10;
    s_psmove_yawsensitivity_slider.generic.name = "Move yaw sensitivity";
    s_psmove_yawsensitivity_slider.generic.callback = PSMoveYawSensitivityFunc;
    s_psmove_yawsensitivity_slider.minvalue = 0;
    s_psmove_yawsensitivity_slider.maxvalue = 30;
    s_psmove_yawsensitivity_slider.curvalue =
        Cvar_VariableValue("psmove_yawsensitivity") * 10.0F;
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_yawsensitivity_slider);

    s_psmove_pitchsensitivity_slider.generic.type = MTYPE_SLIDER;
    s_psmove_pitchsensitivity_slider.generic.x = 0;
    s_psmove_pitchsensitivity_slider.generic.y = y;
    y += 10;
    s_psmove_pitchsensitivity_slider.generic.name = "Move pitch sensitivity";
    s_psmove_pitchsensitivity_slider.generic.callback = PSMovePitchSensitivityFunc;
    s_psmove_pitchsensitivity_slider.minvalue = 0;
    s_psmove_pitchsensitivity_slider.maxvalue = 30;
    s_psmove_pitchsensitivity_slider.curvalue =
        Cvar_VariableValue("psmove_pitchsensitivity") * 10.0F;
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_pitchsensitivity_slider);

    s_psmove_deadzone_slider.generic.type = MTYPE_SLIDER;
    s_psmove_deadzone_slider.generic.x = 0;
    s_psmove_deadzone_slider.generic.y = y;
    y += 10;
    s_psmove_deadzone_slider.generic.name = "Move deadzone";
    s_psmove_deadzone_slider.generic.callback = PSMoveDeadzoneFunc;
    s_psmove_deadzone_slider.minvalue = 0;
    s_psmove_deadzone_slider.maxvalue = 20;
    s_psmove_deadzone_slider.curvalue =
        Cvar_VariableValue("psmove_deadzone") * 100.0F;
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_deadzone_slider);

    s_psmove_smoothing_slider.generic.type = MTYPE_SLIDER;
    s_psmove_smoothing_slider.generic.x = 0;
    s_psmove_smoothing_slider.generic.y = y;
    y += 10;
    s_psmove_smoothing_slider.generic.name = "Move smoothing";
    s_psmove_smoothing_slider.generic.callback = PSMoveSmoothingFunc;
    s_psmove_smoothing_slider.minvalue = 0;
    s_psmove_smoothing_slider.maxvalue = 9;
    s_psmove_smoothing_slider.curvalue =
        Cvar_VariableValue("psmove_smoothing") * 10.0F;
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_smoothing_slider);

    s_psmove_invertpitch_list.generic.type = MTYPE_SPINCONTROL;
    s_psmove_invertpitch_list.generic.x = 0;
    s_psmove_invertpitch_list.generic.y = y;
    y += 10;
    s_psmove_invertpitch_list.generic.name = "invert Move pitch";
    s_psmove_invertpitch_list.generic.callback = PSMoveInvertPitchFunc;
    s_psmove_invertpitch_list.itemnames = no_names;
    s_psmove_invertpitch_list.curvalue =
        (Cvar_VariableValue("psmove_invertpitch") != 0.0F);
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_invertpitch_list);

#ifndef PS3_V198_OSK_RECOVERY
    s_psmove_calibrate_action.generic.type = MTYPE_ACTION;
    s_psmove_calibrate_action.generic.x = 0;
    s_psmove_calibrate_action.generic.y = y;
    y += 10;
    s_psmove_calibrate_action.generic.name = "calibrate / recenter PS Move";
    s_psmove_calibrate_action.generic.callback = PSMoveCalibrateFunc;
    s_psmove_calibrate_action.generic.statusbar =
        "starts Move safely, then opens Eye alignment and calibration";
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_calibrate_action);
#endif

    s_psmove_reinit_action.generic.type = MTYPE_ACTION;
    s_psmove_reinit_action.generic.x = 0;
    s_psmove_reinit_action.generic.y = y;
    y += 10;
    s_psmove_reinit_action.generic.name = "reinitialize PS Move";
    s_psmove_reinit_action.generic.callback = PSMoveReinitFunc;
#ifdef PS3_V198_OSK_RECOVERY
    s_psmove_reinit_action.generic.statusbar =
        "use after reconnecting or changing the active Move controller";
#else
    s_psmove_reinit_action.generic.statusbar =
        "explicit safe start; use after reconnecting or changing the wand";
#endif
    Menu_AddItem(&s_joy_menu, (void *)&s_psmove_reinit_action);
#endif
...
#ifdef __PSL1GHT__
static menuaction_s s_options_remap_ds3_action;
#endif
...
#ifdef __PSL1GHT__
static void
RemapDualShock3Func(void *unused)
{
	M_Menu_ControllerButtons_f();
}
#endif
...
Options_MenuInit(void)
{
    ...
    #ifdef __PSL1GHT__
    s_options_remap_ds3_action.generic.type = MTYPE_ACTION;
    s_options_remap_ds3_action.generic.x = 0;
    s_options_remap_ds3_action.generic.y = 130;
    s_options_remap_ds3_action.generic.name = "remap DualShock 3";
    s_options_remap_ds3_action.generic.callback = RemapDualShock3Func;

    s_options_customize_joy_action.generic.type = MTYPE_ACTION;
    s_options_customize_joy_action.generic.x = 0;
    s_options_customize_joy_action.generic.y = 140;
    s_options_customize_joy_action.generic.name = "gamepad / Move settings";
    ...
    #ifdef __PSL1GHT__
    Menu_AddItem(&s_options_menu, (void *)&s_options_remap_ds3_action);
    #endif
...
M_Credits_MenuDraw(void)
{
            ...
            y += 10, i++)
    {
        ...
        {
            int length = (int)strlen(credits[i] + stringoffset);
            int x = (viddef.width / scale - (int)strlen(credits[i]) * 8 -
                stringoffset * 8) / 2 + stringoffset * 8;

            Draw_GlyphStringScaled(x * scale, y * scale,
                credits[i] + stringoffset, length, scale,
                bold ? 0x80 : 0);
        }
    }
...
#ifdef __PSL1GHT__
    /*
     * The stock newgame alias starts intro.cin before base1. Keep the PS3
     * gameplay path independent of the legacy cinematic decoder: the intro
     * remains available from the console, but selecting a difficulty always
     * reaches the first playable map directly.
     */
    PS3_BOOT_TRACE("StartGame: queued direct base1 launch");
    Cbuf_AddText("loading ; killserver ; wait ; gamemode sp ; maxclients 1 ; map base1\n");
#else
...
static void
AddressBook_CommitField(void *self)
{
    menufield_s *field = (menufield_s *)self;
    char cvar_name[20];
    int index = field->generic.localdata[0];

    if (index < 0 || index >= NUM_ADDRESSBOOK_ENTRIES)
    {
        return;
    }

    Com_sprintf(cvar_name, sizeof(cvar_name), "adr%d", index);
    Cvar_Set(cvar_name, field->buffer);
    /* The PS3 may be powered down without another orderly client transition.
     * Persist an accepted OSK result immediately instead of waiting for exit. */
    CL_WriteConfiguration();
    Com_Printf("Address book entry %d saved: %s\n", index + 1,
        field->buffer[0] ? field->buffer : "<empty>");
}

static void
AddressBook_CommitAll(void)
{
    int index;
    char cvar_name[20];
    ...
    {
        Com_sprintf(cvar_name, sizeof(cvar_name), "adr%d", index);
        Cvar_Set(cvar_name, s_addressbook_fields[index].buffer);
    }
    CL_WriteConfiguration();
}
...
AddressBook_MenuInit(void)
{
    ...
    for (i = 0; i < NUM_ADDRESSBOOK_ENTRIES; i++)
    {
        ...
        s_addressbook_fields[i].generic.callback = AddressBook_CommitField;
		s_addressbook_fields[i].generic.flags = QMF_OSK_ADDRESS;
		s_addressbook_fields[i].generic.statusbar =
			"Cross edits; Circle saves all entries; Refresh checks saved addresses";
        ...
        Q_strlcpy(s_addressbook_fields[i].buffer, adr->string,
            sizeof(s_addressbook_fields[i].buffer));
...
AddressBook_MenuKey(int key)
{
    ...
    {
        /* Circle/Select are translated to Escape by Default_MenuKey. Commit on
         * that translated value too; the old raw-key check silently discarded
         * every PS3 controller edit when leaving this screen. */
        AddressBook_CommitAll();
    }
