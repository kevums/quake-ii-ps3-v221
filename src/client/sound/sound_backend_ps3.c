...
static volatile int snd_inited = 0;
static int snd_scaletable[32][256];
static int snd_vol;
static int soundtime;
static int sound_buffers;
...
static volatile int bufferClearLock = 0;
static sys_ppu_thread_t audioThreadId;

static volatile int audioThreadRunning;
static volatile u32 audioBlocksCopied;
static volatile u32 audioBlocksWithSamples;
static volatile u32 audioSilentBlocks;
static int audioThreadCreated;
static int audioLibraryInitialized;
static int audioPortOpened;
static int audioPortStarted;
static int audioQueueCreated;
static int audioQueueAttached;
static u64 snd_key;
...
SB_ClearBuffer(void)
{
	if (sound_started == SS_NOT || !sound.buffer)
	{
		return;
	}
...
SB_UpdateSoundtime(void)
{
	...
	soundtime = sound_buffers * fullsamples + playpos / sound.channels;
	sound.samplepos = playpos;
}
...
SB_Update(void)
{
	...
	if (!soundtime)
	{
		/* The first hardware block may not have advanced yet. Never retain the
		 * writer lock across frames: doing so permanently starves the PS3 audio
		 * thread and was the reason the old backend stayed silent after startup. */
		bufferClearLock = 0;
		return;
	}
...
SB_SoundInfo(void)
{
	...
	Com_Printf("%5u PS3 hardware blocks copied\n",
		(unsigned int)audioBlocksCopied);
	Com_Printf("%5u PS3 blocks containing mixed samples\n",
		(unsigned int)audioBlocksWithSamples);
	Com_Printf("%5u PS3 zero blocks during mixer updates\n",
		(unsigned int)audioSilentBlocks);
}
...
SB_PS3_shutdownAudioLib(void)
{
	...
	if (backend && backend->buffer != NULL)
	{
		free(backend->buffer);
		backend->buffer = NULL;
	}
	...
	if (audioQueueCreated)
	{
		ret = sysEventQueueDestroy(snd_queue, 0);
		printf("  sysEventQueueDestroy: %X\n", ret);
		audioQueueCreated = 0;
	}
	if (audioLibraryInitialized)
	{
		ret = audioQuit();
		printf("  audioQuit: %X\n", ret);
		audioLibraryInitialized = 0;
	}
}
...
{
	const s16 *pcm = (const s16 *)backend->buffer;
	int position = playpos;
	int mask = backend->samples - 1;
	unsigned int frame;
	qboolean has_samples = false;

	/* The backend is fixed at signed 16-bit stereo and the software ring is a
	 * power of two. Work in mono-sample indices, matching Yamagi's sound_t, and
	 * publish the cursor only after the complete hardware block is copied. */
	for (frame = 0; frame < AUDIO_BLOCK_SAMPLES; frame++)
	{
		s16 left = pcm[position & mask];
		block[frame * 2 + 0] = (f32)left / 32768.0f;
		position++;
		s16 right = pcm[position & mask];
		block[frame * 2 + 1] = (f32)right / 32768.0f;
		position++;
		has_samples |= (left != 0 || right != 0);
	}

	playpos = position & mask;
	backend->samplepos = playpos;
	audioBlocksCopied++;
	if (has_samples)
	{
		audioBlocksWithSamples++;
	}
}

static void
SB_PS3_advanceSilentBlock(void)
{
	playpos = (playpos + AUDIO_BLOCK_SAMPLES * backend->channels) &
		(backend->samples - 1);
	backend->samplepos = playpos;
	audioSilentBlocks++;
}
...
SB_PS3_audioThread(void * arg)
{
	...
	while (snd_inited)
	{
		/* Audio notifications normally arrive every 5.33 ms. A delayed event is
		 * recoverable; do not permanently shut audio down because the PPU was busy
		 * loading a map or servicing RSX. */
		if (sysEventQueueReceive(snd_queue, &event, 100 * 1000) != 0)
		{
			if (snd_inited && timeout_count++ == 0)
			{
				printf("Audio notify timeout; continuing with silence until recovery.\n");
			}
			continue;
		}
		timeout_count = 0;
		...
		/* Sample the hardware cursor after the notification. Reading it before
		 * waiting can select the previous block and replay data one event late. */
		...
		{
			/* Never replay an old hardware block while the software ring is being
			 * updated. A brief zero block is preferable to stale or torn PCM. */
			memset(block, 0, config.channelCount * AUDIO_BLOCK_SAMPLES *
				sizeof(*block));
			SB_PS3_advanceSilentBlock();
		}
	}
...
{
	PS3_RUNTIME_TRACE("SB_PS3_BackendShutdown: entered");
	...
	if (audioThreadCreated)
	{
		...
		PS3_RUNTIME_TRACE("SB_PS3_BackendShutdown: audio thread joined");
	}
	else
	{
		SB_PS3_shutdownAudioLib();
	...
	snd_inited = 0;	
	PS3_RUNTIME_TRACE("SB_PS3_BackendShutdown: complete");
}
...
SB_PS3_BackendInit(void)
{
	...
	PS3_BOOT_TRACE("SB_PS3_BackendInit: before audioInit");
	if (audioInit() != 0)
	{
		Com_Printf("audioInit() != 0\n");
		return false;
	}
	audioLibraryInitialized = 1;
	PS3_BOOT_TRACE("SB_PS3_BackendInit: audioInit complete");
	...
	PS3_BOOT_TRACE("SB_PS3_BackendInit: before audioPortOpen");
	...
	audioPortOpened = 1;
	PS3_BOOT_TRACE("SB_PS3_BackendInit: audioPortOpen complete");
	printf("  portNum: %d\n", portNum);

	// Geting info on opened port
	PS3_BOOT_TRACE("SB_PS3_BackendInit: before audioGetPortConfig");
	...
	PS3_BOOT_TRACE("SB_PS3_BackendInit: audioGetPortConfig complete");
	...
	PS3_BOOT_TRACE("SB_PS3_BackendInit: before audio event queue");
	...
	audioQueueAttached = 1;

	ret = sysEventQueueDrain(snd_queue);
	Com_Printf("  sysEventQueueDrain: %08x\n", ret);

	/* Clear every hardware block before the port starts. This prevents startup
	 * noise and gives the software mixer one complete hardware-ring interval to
	 * establish its normal mix-ahead distance. */
	memset((void *)(uintptr_t)config.audioDataStart, 0,
		(size_t)config.channelCount * AUDIO_BLOCK_SAMPLES * config.numBlocks *
		sizeof(f32));

	PS3_BOOT_TRACE("SB_PS3_BackendInit: before audioPortStart");
	...
	audioPortStarted = 1;
	PS3_BOOT_TRACE("SB_PS3_BackendInit: audioPortStart complete");
	...
	/// to backend's buffer ()
	/* A 170 ms, power-of-two software ring gives the 60 Hz main thread ample
	 * mix-ahead while the hardware consumes fixed 5.33 ms blocks. */
	backend->samples = AUDIO_BLOCK_SAMPLES * config.numBlocks *
	...
	if (!backend->buffer)
	{
		Com_Printf("Unable to allocate PS3 software audio ring.\n");
		SB_PS3_shutdownAudioLib();
		return false;
	}
	...
	snd_inited = 1;
	PS3_BOOT_TRACE("SB_PS3_BackendInit: before audio thread");
	...
	PS3_BOOT_TRACE("SB_PS3_BackendInit: audio thread complete");
	...
	PS3_BOOT_TRACE("SB_PS3_BackendInit: complete");
	return 1;
}
