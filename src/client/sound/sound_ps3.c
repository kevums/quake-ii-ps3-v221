...
S_Init(void)
{
	...
	PS3_BOOT_TRACE("S_Init: before SB_PS3_BackendInit");
	if (SB_PS3_BackendInit() == false)
	{
		PS3_BOOT_TRACE("S_Init: backend initialization failed");
		sound_started = SS_NOT;
		return;
	}
	PS3_BOOT_TRACE("S_Init: backend initialization complete");
	sound_started = 1;
	...
	S_StopAllSounds();
	PS3_BOOT_TRACE("S_Init: complete");
...
S_Shutdown(void)
{
	int i;
	sfx_t *sfx;
	PS3_RUNTIME_TRACE("S_Shutdown: entered");

	if (sound_started == SS_NOT)
	{
		PS3_RUNTIME_TRACE("S_Shutdown: backend was not active");
		return;
	}
	...
	PS3_RUNTIME_TRACE("S_Shutdown: complete");
}
