...
CL_Init(void)
{
	PS3_BOOT_TRACE("CL_Init: entered");
	...
	PS3_BOOT_TRACE("CL_Init: before Con_Init");
	Con_Init();

	PS3_BOOT_TRACE("CL_Init: before S_Init");
	S_Init();
	PS3_BOOT_TRACE("CL_Init: S_Init complete");

	SCR_Init();

	PS3_BOOT_TRACE("CL_Init: before VID_Init");
	VID_Init();
	PS3_BOOT_TRACE("CL_Init: VID_Init complete");

	PS3_BOOT_TRACE("CL_Init: before IN_Init");
	IN_Init();
	PS3_BOOT_TRACE("CL_Init: IN_Init complete");
	...
	M_Init();
	PS3_BOOT_TRACE("CL_Init: M_Init complete");
	...
	PS3_BOOT_TRACE("CL_Init: complete");
}
...
CL_Shutdown(void)
{
	...
	PS3_RUNTIME_TRACE("CL_Shutdown: entered");

	if (isdown)
	{
		printf("recursive shutdown\n");
		PS3_RUNTIME_TRACE("CL_Shutdown: recursive call ignored");
		return;
	}
	...
	PS3_RUNTIME_TRACE("CL_Shutdown: before configuration save");
	CL_WriteConfiguration();

	Key_WriteConsoleHistory();
	PS3_RUNTIME_TRACE("CL_Shutdown: configuration save complete");

	OGG_Stop();

	PS3_RUNTIME_TRACE("CL_Shutdown: before sound shutdown");
	S_Shutdown();
	PS3_RUNTIME_TRACE("CL_Shutdown: sound shutdown complete");
	PS3_RUNTIME_TRACE("CL_Shutdown: before input shutdown");
	IN_Shutdown();
	PS3_RUNTIME_TRACE("CL_Shutdown: input shutdown complete");
	PS3_RUNTIME_TRACE("CL_Shutdown: before video shutdown");
	VID_Shutdown();
	PS3_RUNTIME_TRACE("CL_Shutdown: complete");
}
