...
Mod_LoadTexinfo(model_t *loadmodel, byte *mod_base, lump_t *l)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		out->ps3_cached_image = NULL;
		out->ps3_cached_frame = 0;
		out->ps3_cached_registration =
			(out->flags & (SURF_TRANS33 | SURF_TRANS66)) ? -1 : 0;
		#endif
		...
		for (step = out->next; step && step != out; step = step->next)
		{
			out->numframes++;
			#ifdef PS3_NATIVE_RSX
			if (step->flags & (SURF_TRANS33 | SURF_TRANS66))
			{
				out->ps3_cached_registration = -1;
			}
			#endif
		}
	}
...
static int calcTexinfoAndFacesSize(byte *mod_base, const lump_t *fl, const lump_t *tl)
{
	...
	for (int surfnum = 0; surfnum < face_count; surfnum++, face_in++)
	{
		...
		else
		{
			...
			#ifdef PS3_NATIVE_RSX
			/* Persistent BSP fans append their immutable 16-bit triangle list
			 * after the variable vertex payload. */
			if (numverts >= 3)
				polySize += (numverts - 2) * 3 * sizeof(unsigned short);
			#endif
...
Mod_LoadFaces(model_t *loadmodel, byte *mod_base, lump_t *l)
{
	...
	for (surfnum = 0; surfnum < count; surfnum++, in++, out++)
	{
		...
		out->polys = NULL;
		#ifdef PS3_NATIVE_RSX
		out->ps3_lightstyle_count = 0;
		out->ps3_combined_geometry_eligible = 0;
		#endif
		...
		#ifdef PS3_NATIVE_RSX
		for (i = 0; i < MAXLIGHTMAPS && out->styles[i] != 255; i++)
		{
			out->ps3_lightstyle_count++;
		}
		#endif
...
RI_BeginRegistration(char *model)
{
	...
	{
		#ifdef PS3_NATIVE_RSX
		RSXGL_ResetStaticWorldGeometry();
		#endif
...
RI_EndRegistration(void)
{
	...
	#ifdef PS3_NATIVE_RSX
	/* Publish all one-time local BSP writes before any early return. */
	RSXGL_FinalizeStaticWorldGeometry();
	#endif
