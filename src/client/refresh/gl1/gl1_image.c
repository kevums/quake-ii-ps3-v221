...
#ifdef __PSL1GHT__
/* Texture registration runs on the constrained PPU main-thread stack. The
 * desktop GL uploader's two 256x256 scratch arrays consume about 320 KiB, so
 * keep one persistent, renderer-owned workspace on PS3. */
static unsigned ps3_upload_scaled[256 * 256];
static unsigned char ps3_upload_paletted[256 * 256];
#endif
...
float
R_GetUploadGamma(void)
{
#if defined(__PSL1GHT__) && defined(PS3_NATIVE_RSX)
	float gamma = vid_gamma ? vid_gamma->value : 1.0f;

	/* The PS3 video menu exposes 0.1 through 2.0. Keep console input inside
	 * the same useful range and turn NaN/non-positive input into identity. */
	if (!(gamma > 0.0f))
	{
		gamma = 1.0f;
	}
...
static void
R_BuildGammaTable(void)
{
	int i;

#if defined(__PSL1GHT__) && defined(PS3_NATIVE_RSX)
	float gamma = R_GetUploadGamma();
	/* Match SDL_CalculateGammaRamp(), used by the desktop GL1 renderer. */
	float exponent = 1.0f / gamma;

	for (i = 0; i < 256; i++)
	{
		float corrected = 255.0f * powf((float)i / 255.0f, exponent);
		int value = (int)(corrected + 0.5f);
		...
		gammatable[i] = (unsigned char)value;
	}
	PS3_BOOT_TRACE("RSX renderer: texture-upload gamma table rebuilt");
#else
	for (i = 0; i < 256; i++)
	{
		gammatable[i] = i;
...
R_TextureMode(char *string)
{
	...
	for (i = 0, glt = gltextures; i < numgltextures; i++, glt++)
	{
		...
		/* The console font is a tightly packed atlas. Never let a texture-mode
		 * change re-enable cross-cell interpolation after Draw_InitLocal fixed it. */
		if (!strcmp(glt->name, "pics/conchars.pcx"))
		{
			nolerp = true;
		}
		/* r_2D_unfiltered and r_nolerp_list allow rendering stuff unfiltered even if gl_filter_* is filtered */
		else if (unfiltered2D && glt->type == it_pic)
		{
...
R_Upload32Soft(unsigned *data, int width, int height, qboolean mipmap)
{
	int samples;
	#ifdef __PSL1GHT__
	unsigned *scaled = ps3_upload_scaled;
	unsigned char *paletted_texture = ps3_upload_paletted;
	#else
...
R_InitImages(void)
{
	...
	#if defined(__PSL1GHT__) && defined(PS3_NATIVE_RSX)
	{
		char lighting_trace[192];
		snprintf(lighting_trace, sizeof(lighting_trace),
			"RSX lighting: gamma %.3f intensity %.3f modulate %.3f fullbright %.0f overbright %.3f",
			R_GetUploadGamma(), intensity->value,
			r_modulate ? r_modulate->value : 1.0f,
			r_fullbright ? r_fullbright->value : 0.0f,
			gl1_overbrightbits ? gl1_overbrightbits->value : 0.0f);
		PS3_RUNTIME_TRACE(lighting_trace);
	}
	#endif
}
