...
/* Static lightmaps use 1..126; the legacy MAX_LIGHTMAPS overflow guard leaves
 * 127 unused. Give the paired stereo eye its own dynamic atlas there so it can
 * be updated before sampling instead of renaming eye one's live texture. */
#define PS3_STEREO_DYNAMIC_LIGHTMAP (MAX_LIGHTMAPS - 1)
#endif
...
#ifdef PS3_NATIVE_RSX
extern qboolean r_ps3_second_stereo_eye;
qboolean R_PS3_StereoActive(void);
qboolean R_PS3_StereoLightmapReuseEnabled(void);
qboolean R_PS3_StereoLightmapAtlasEnabled(void);
void R_PS3_BeginAliasStereoEye(float camera_separation,
	qboolean second_eye);
void R_PS3_EndAliasStereoPair(void);
void R_PS3_EndVisualFrame(void);
#endif
