...
qboolean r_ps3_second_stereo_eye;
static qboolean ps3_visual_frame_open;

static inline unsigned long long
R_PS3_ProfileStageBegin(qboolean enabled)
{
	return enabled ? (unsigned long long)Sys_Microseconds() : 0;
}

static inline void
R_PS3_ProfileStageEnd(qboolean enabled, enum rsxgl_render_stage stage,
	unsigned long long started_us)
{
	if (enabled)
	{
		RSXGL_RecordRenderStage(stage,
			(unsigned long long)Sys_Microseconds() - started_us);
	}
}
#endif
...
#ifdef PS3_NATIVE_RSX
static cvar_t *ps3_native_stereo_enable;
static cvar_t *ps3_native_stereo_lightmap_reuse;
static cvar_t *ps3_native_stereo_lightmap_atlas;
#endif

#ifdef __PSL1GHT__
/* Particle counts are bounded by MAX_PARTICLES. Reuse one renderer-owned set
 * instead of allocating up to roughly 440 KiB from the PPU heap every frame;
 * point particles use prefixes of the same vertex and color workspaces. */
static GLfloat ps3_particle_vtx[3 * MAX_PARTICLES * 3];
static GLfloat ps3_particle_tex[2 * MAX_PARTICLES * 3];
#ifdef PS3_NATIVE_RSX
/* The typed native submission resolves one color per particle and applies it
 * to all three corners, matching the legacy repeated array without retaining
 * two redundant copies in PPU memory. */
static GLfloat ps3_particle_clr[4 * MAX_PARTICLES];
#else
static GLfloat ps3_particle_clr[4 * MAX_PARTICLES * 3];
#endif
#ifdef PS3_NATIVE_RSX
static entity_t *ps3_translucent_entities[MAX_ENTITIES];
/* Particle texture coordinates never vary, and a stereo pair uses the same
 * simulation snapshot, view axes, and forward distance for both eyes. Keep
 * the first eye's completed arrays until the immediately following eye rather
 * than rebuilding them on the PPU. The RSX backend still packs each eye into
 * its own append-only vertex range and submits it with its own matrix. */
static const particle_t *ps3_particle_cache_source;
static int ps3_particle_cache_count;
static int ps3_particle_cache_mode;
static int ps3_particle_tex_initialized;
static qboolean ps3_particle_cache_valid;

static void
R_PS3_NoteParticleStereoReuse(void)
{
	static qboolean traced;
	if (!traced)
	{
		PS3_RUNTIME_TRACE("GL1 renderer: stereo particle expansion reused on second eye");
		traced = true;
	}
}
#endif
...
R_PS3_BoxOutsidePlane(const vec3_t mins, const vec3_t maxs,
	const cplane_t *plane)
{
	float distance;

	/* BOX_ON_PLANE_SIDE computes both the maximum and minimum projected
	 * corners so callers can distinguish front, back, and spanning boxes.
	 * Frustum rejection only asks whether even the maximum corner is behind
	 * the plane. Select that support corner directly and perform one dot
	 * product instead of two for every visited BSP node. */
	switch (plane->signbits & 7)
	{
		case 0:
			distance = plane->normal[0] * maxs[0] +
				plane->normal[1] * maxs[1] +
				plane->normal[2] * maxs[2];
			break;
		case 1:
			distance = plane->normal[0] * mins[0] +
				plane->normal[1] * maxs[1] +
				plane->normal[2] * maxs[2];
			break;
		case 2:
...
R_CullBox(vec3_t mins, vec3_t maxs)
{
	...
	{
		#ifdef PS3_NATIVE_RSX
		if (R_PS3_BoxOutsidePlane(mins, maxs, &frustum[i]))
		#else
...
{
	if (e->origin[0] != 0.0f || e->origin[1] != 0.0f ||
		e->origin[2] != 0.0f)
	{
...
R_DrawSpriteModel(entity_t *currententity, const model_t *currentmodel)
{
	...
	VectorMA(currententity->origin, frame->height - frame->origin_y, up,
		point[1]);
...
#ifdef PS3_NATIVE_RSX
	{
		...
		VectorCopy(point[0], mins);
		VectorCopy(point[0], maxs);
		for (corner = 1; corner < 4; corner++)
		{
			for (axis = 0; axis < 3; axis++)
			{
				if (point[corner][axis] < mins[axis])
					mins[axis] = point[corner][axis];
				if (point[corner][axis] > maxs[axis])
					maxs[axis] = point[corner][axis];
			}
		}
...
#ifdef PS3_NATIVE_RSX
	RSXGL_DrawTexturedFan(point[0], 3, tex, 2, 4);
#else
...
static void
R_DrawEntity(entity_t *currententity)
{
	...
	{
		case mod_alias:
			R_DrawAliasModel(currententity, currententity->model);
			break;
		case mod_brush:
			R_DrawBrushModel(currententity, currententity->model);
			break;
		case mod_sprite:
			R_DrawSpriteModel(currententity, currententity->model);
			break;
		default:
...
R_DrawEntitiesOnList(void)
{
	...
	for (i = 0; i < r_newrefdef.num_entities; i++)
	{
		...
		{
			#ifdef PS3_NATIVE_RSX
			ps3_translucent_entities[translucent_count++] = currententity;
			#endif
			continue;
		}
		R_DrawEntity(currententity);
	}
	...
	glDepthMask(0);
	for (i = 0; i < translucent_count; i++)
	{
		R_DrawEntity(ps3_translucent_entities[i]);
	}
#else
	glDepthMask(0);
		...
		const unsigned colortable[768])
{
	...
	#ifdef __PSL1GHT__
	GLfloat *vtx = ps3_particle_vtx;
	GLfloat *tex = ps3_particle_tex;
	GLfloat *clr = ps3_particle_clr;
	#else
	...
	#endif
	#ifdef PS3_NATIVE_RSX
	qboolean reuse_arrays = num_particles > 0 && r_ps3_second_stereo_eye &&
		ps3_particle_cache_valid && ps3_particle_cache_mode == 2 &&
		ps3_particle_cache_source == particles &&
		ps3_particle_cache_count == num_particles;
	#endif
	...
	if (!reuse_arrays)
	{
		/* The same three atlas coordinates are used by every expanded
		 * particle. Initialize only a newly reached suffix, once. */
		for (i = ps3_particle_tex_initialized; i < num_particles; i++)
		{
			index_tex = (unsigned int)i * 6;
		...
		if (num_particles > ps3_particle_tex_initialized)
		{
			ps3_particle_tex_initialized = num_particles;
		}
		index_tex = 0;
	#endif
	...
	for ( p = particles, i = 0; i < num_particles; i++, p++ )
	{
		...
		*(unsigned *) color = colortable [ p->color ];
		...
		#ifdef PS3_NATIVE_RSX
		/* All three corners use the same color. The typed RSX path expands it
		 * while writing the final hardware vertex stream. */
		...
		ps3_particle_cache_source = particles;
		ps3_particle_cache_count = num_particles;
		ps3_particle_cache_mode = 2;
		ps3_particle_cache_valid = true;
	}
	else
	{
		R_PS3_NoteParticleStereoReuse();
	}
	#endif

	#ifdef PS3_NATIVE_RSX
	RSXGL_DrawParticleTriangles(vtx, tex, clr, num_particles);
	#else
	glEnableClientState( GL_VERTEX_ARRAY );
	glEnableClientState( GL_TEXTURE_COORD_ARRAY );
...
R_DrawParticles(void)
{
	...
	if (gl_config.pointparameters && !(stereo_split_tb || stereo_split_lr))
	{
		...
		unsigned int index_clr = 0;
		#ifdef PS3_NATIVE_RSX
		qboolean reuse_arrays = r_newrefdef.num_particles > 0 &&
			r_ps3_second_stereo_eye &&
			ps3_particle_cache_valid && ps3_particle_cache_mode == 1 &&
			ps3_particle_cache_source == r_newrefdef.particles &&
			ps3_particle_cache_count == r_newrefdef.num_particles;
		#endif
...
#ifndef PS3_POLYBLEND_2D_OVERRIDE
void
R_PolyBlend(void)
{
...
{
	int i;
	float horizontal_fov = r_newrefdef.fov_x;
	#ifdef PS3_NATIVE_RSX
	if (ps3_native_stereo_enable && ps3_native_stereo_enable->value > 0.0f &&
		gl_state.camera_separation != 0.0f)
	{
		/* R_MYgluPerspective() shifts each eye's projection horizontally. The
		 * legacy CPU frustum remains centered, which can reject edge geometry
		 * visible to one eye and expose the clear color. Expand both side planes
		 * by the same normalized near-plane shift (zNear is four below). */
		const float z_near = 4.0f;
		float half_width = tanf(horizontal_fov * (float)M_PI / 360.0f);
		float shift = fabsf(gl1_stereo_convergence->value *
			(2.0f * gl_state.camera_separation)) / (z_near * z_near);
		horizontal_fov = atanf(half_width + shift) * 360.0f /
			(float)M_PI;
	}
	#endif
...
R_Clear(void)
{
	...
	qboolean forceColorClear = false;
	qboolean acquiredDepthClear = false;
	#ifdef PS3_NATIVE_RSX
	/* RSXGL_BeginFrame clears the complete newly acquired color/depth/stencil
	 * target under a known full scissor. Keep R_Clear responsible for explicit
	 * r_clear diagnostics and special stencil modes, avoiding redundant color
	 * and depth clears here (or one duplicate per eye in frame-packed 3D). */
	acquiredDepthClear = (gl1_ztrick->value == 0.0f);
	#endif
	if (forceColorClear && !r_clear->value)
	{
		/* Magenta is useful for r_clear diagnostics, but uncovered pixels in
		 * native play should begin from stable black. */
	...
	if (gl1_ztrick->value)
	{
		...
		if (r_clear->value || forceColorClear)
		{
	...
	else
	{
		...
		else if (!acquiredDepthClear)
		{
			glClear(GL_DEPTH_BUFFER_BIT | stencilFlags);
		}
		else if (stencilFlags)
		{
			glClear(stencilFlags);
		}
...
R_SetGL2D(void)
{
	...
	glViewport(x, y, w, h);
	#ifdef PS3_NATIVE_RSX
	RSXGL_Set2DProjection(vid.width, vid.height);
	#else
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
...
{
	#ifdef PS3_NATIVE_RSX
	qboolean ps3_profile_enabled;
	unsigned long long ps3_profile_started_us;
	#endif
	...
	#ifdef PS3_NATIVE_RSX
	ps3_profile_enabled = RSXGL_StatsEnabled() ? true : false;
	ps3_profile_started_us = R_PS3_ProfileStageBegin(ps3_profile_enabled);
	if (!r_ps3_second_stereo_eye)
	{
		R_PushDlights();
	}
	else if (r_newrefdef.num_dlights > 0)
	{
		static qboolean traced_stereo_dlight_reuse;
		if (!traced_stereo_dlight_reuse)
		{
			PS3_RUNTIME_TRACE(
				"RSX renderer: stereo dynamic-light marks reused on second eye");
			traced_stereo_dlight_reuse = true;
		}
	}
	#else
	R_PushDlights();
	#endif
	#ifdef PS3_NATIVE_RSX
	R_PS3_ProfileStageEnd(ps3_profile_enabled,
		RSXGL_RENDER_STAGE_DLIGHTS, ps3_profile_started_us);
	ps3_profile_started_us = R_PS3_ProfileStageBegin(ps3_profile_enabled);
	#endif
	...
	#ifdef PS3_NATIVE_RSX
	R_PS3_ProfileStageEnd(ps3_profile_enabled,
		RSXGL_RENDER_STAGE_SETUP, ps3_profile_started_us);
	ps3_profile_started_us = R_PS3_ProfileStageBegin(ps3_profile_enabled);
	#endif
	...
	#ifdef PS3_NATIVE_RSX
	R_PS3_ProfileStageEnd(ps3_profile_enabled,
		RSXGL_RENDER_STAGE_WORLD, ps3_profile_started_us);
	ps3_profile_started_us = R_PS3_ProfileStageBegin(ps3_profile_enabled);
	#endif
	...
	#ifdef PS3_NATIVE_RSX
	R_PS3_ProfileStageEnd(ps3_profile_enabled,
		RSXGL_RENDER_STAGE_ENTITIES, ps3_profile_started_us);
	ps3_profile_started_us = R_PS3_ProfileStageBegin(ps3_profile_enabled);
	#endif
	...
	#ifdef PS3_NATIVE_RSX
	R_PS3_ProfileStageEnd(ps3_profile_enabled,
		RSXGL_RENDER_STAGE_EFFECTS, ps3_profile_started_us);
	ps3_profile_started_us = R_PS3_ProfileStageBegin(ps3_profile_enabled);
	#endif
	...
	#ifdef PS3_NATIVE_RSX
	R_PS3_ProfileStageEnd(ps3_profile_enabled,
		RSXGL_RENDER_STAGE_ALPHA, ps3_profile_started_us);
	#endif
...
R_Register(void)
{
	...
	gl1_dynamic = ri.Cvar_Get("gl1_dynamic", "1", CVAR_ARCHIVE);
	...
	gl1_polyblend = ri.Cvar_Get("gl1_polyblend", "1", CVAR_ARCHIVE);
	...
	#ifdef PS3_NATIVE_RSX
	{
		cvar_t *console_scale_revision =
			ri.Cvar_Get("ps3_console_scale_revision", "0", CVAR_ARCHIVE);
		/* The corrected addressable text path exposes the full archived HD
		 * automatic scale which the corrupt inline geometry partially hid.
		 * Keep this migration in the renderer so the focused recovery build does
		 * not need cl_screen's newer Move-overlay dependencies. */
		if (console_scale_revision->value < 3.0f)
		{
			ri.Cvar_Set("r_consolescale", "1");
			ri.Cvar_Set("ps3_console_scale_revision", "3");
		}
	}
	ps3_native_stereo_enable =
		ri.Cvar_Get("ps3_stereo_enable", "0", CVAR_ARCHIVE);
	ps3_native_stereo_lightmap_reuse =
		ri.Cvar_Get("ps3_rsx_stereo_lightmap_reuse", "1", CVAR_ARCHIVE);
	ps3_native_stereo_lightmap_atlas =
		ri.Cvar_Get("ps3_rsx_stereo_lightmap_atlas", "1", CVAR_ARCHIVE);
	#endif
...
{
	#ifdef PS3_NATIVE_RSX
	static qboolean traced_first_begin_frame;
	if (!ps3_visual_frame_open)
	{
		ps3_visual_frame_open = true;
		r_ps3_second_stereo_eye = false;
		ps3_particle_cache_valid = false;
	}
	else
	{
		/* SCR_UpdateScreen submits the second eye as the only additional
		 * BeginFrame before the shared EndFrame. Use that lifecycle rather than
		 * separation signs: a valid stereo depth of zero makes both values zero
		 * but still renders two eye surfaces. */
		r_ps3_second_stereo_eye = true;
	}
	R_PS3_BeginAliasStereoEye(camera_separation,
		r_ps3_second_stereo_eye);
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: entered");
	}
	/* Stereo calls this twice. RSXGL_BeginFrame is idempotent until EndFrame,
	 * so the first pass always acquires the target regardless of separation
	 * sign or a zero depth setting. */
	RSXGL_BeginFrame();
	RSXGL_SetStereoEye(camera_separation);
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: native frame acquired");
	}
	#endif
	...
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: cvar state complete");
	}
	#endif
	...
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: before viewport");
	}
	#endif
	glViewport(x, y, w, h);
	#ifdef PS3_NATIVE_RSX
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: viewport complete");
	}
	#endif
	#ifdef PS3_NATIVE_RSX
	RSXGL_Set2DProjection(vid.width, vid.height);
	...
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: matrices complete");
	}
	#endif
	glDisable(GL_DEPTH_TEST);
	glDisable(GL_CULL_FACE);
	...
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: fixed state complete");
	}
	#endif
		...
		|| r_2D_unfiltered->modified || r_videos_unfiltered->modified)
	{
		...
		if (!traced_first_begin_frame)
		{
			PS3_BOOT_TRACE("GL1 first BeginFrame: before texture mode refresh");
		}
		#endif
		...
		if (!traced_first_begin_frame)
		{
			PS3_BOOT_TRACE("GL1 first BeginFrame: texture mode refresh complete");
		}
		#endif
	}
	...
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: before R_Clear");
	}
	#endif
	R_Clear();
	#ifdef PS3_NATIVE_RSX
	if (!traced_first_begin_frame)
	{
		PS3_BOOT_TRACE("GL1 first BeginFrame: R_Clear complete");
		PS3_BOOT_TRACE("GL1 first BeginFrame: complete");
		traced_first_begin_frame = true;
	}
	#endif
}
...
#ifdef PS3_NATIVE_RSX
qboolean
R_PS3_StereoActive(void)
{
	return ps3_native_stereo_enable &&
		ps3_native_stereo_enable->value > 0.0f;
}

qboolean
R_PS3_StereoLightmapReuseEnabled(void)
{
	return R_PS3_StereoActive() && ps3_native_stereo_lightmap_reuse &&
		ps3_native_stereo_lightmap_reuse->value > 0.0f;
}

qboolean
R_PS3_StereoLightmapAtlasEnabled(void)
{
	return R_PS3_StereoActive() && ps3_native_stereo_lightmap_atlas &&
		ps3_native_stereo_lightmap_atlas->value > 0.0f;
}

void
R_PS3_EndVisualFrame(void)
{
	R_PS3_EndAliasStereoPair();
	ps3_visual_frame_open = false;
	r_ps3_second_stereo_eye = false;
	ps3_particle_cache_valid = false;
}
#endif
...
R_DrawBeam(entity_t *e)
{
...
#ifdef PS3_NATIVE_RSX
	{
		vec3_t mins;
		vec3_t maxs;
		float radius = e->frame >= 0 ? e->frame * 0.5f : -e->frame * 0.5f;
		int axis;

		for (axis = 0; axis < 3; axis++)
		{
			mins[axis] = (origin[axis] < oldorigin[axis] ?
				origin[axis] : oldorigin[axis]) - radius;
			maxs[axis] = (origin[axis] > oldorigin[axis] ?
				origin[axis] : oldorigin[axis]) + radius;
		}
		if (R_CullBox(mins, maxs))
		{
...
extern void RDraw_GlyphStringScaled(int x, int y, const char *text, int count,
...
GetRefAPI(refimport_t imp)
{
	...
	re.DrawGlyphStringScaled = RDraw_GlyphStringScaled;
...
#ifndef UNICORE
/*
 * This is only needed by the standalone renderer library. A UNICORE console
 * build already links the engine's authoritative implementations.
 */
void
