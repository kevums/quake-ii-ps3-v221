 ...
 *     Yamagi's aproach tightly bound to SDL. I've tried to separate
 *     image -preparing and -drawing parts (backend and frontend) to
 *     sw_ps3_main.c and sw_ps3_gcm.c accordingly. It's posible to
 *     reflect my work in sw_ps3_gcm.c to SDL for use on other platforms.
 *     Main thing is the bound with sw_ps3_gcm.c (or any other frontend
 *     part) with vid_ps3.c (vid.c).
 * 
 *     sw_ps3_main.c contains only frontend independent parts of
 *     software renderer.
 *     local_ps3.h contains definitions of functions shared between
 *     frontend and backend.
 *     fixes_ps3.h getTicks abstraction (can be done with or w/o SDL).
 * 
 * -----------------------------------------------------------------------
 */
...
pixel_t *stereo_left_frame = NULL;
int	swap_current = 0;
int ps3_stereo_eye = -1;
...
RE_Init(void)
{
	PS3_BOOT_TRACE("RE_Init: entered");
	R_RegisterVariables ();
	PS3_BOOT_TRACE("RE_Init: before R_InitImages");
	R_InitImages ();
	PS3_BOOT_TRACE("RE_Init: R_InitImages complete");
	Mod_Init ();
	Draw_InitLocal ();
	PS3_BOOT_TRACE("RE_Init: Draw_InitLocal complete");
	...
	Draw_GetPalette ();
	PS3_BOOT_TRACE("RE_Init: palette loaded");

	/* set our "safe" mode */
	sw_state.prev_mode = 4;

	/* create the window and set up the context */
	PS3_BOOT_TRACE("RE_Init: before RE_SetMode");
	if (!RE_SetMode())
	{
		R_Printf(PRINT_ALL, "%s() could not R_SetMode()\n", __func__);
		return false;
	}
	PS3_BOOT_TRACE("RE_Init: RE_SetMode complete");
	...
	PS3_BOOT_TRACE("RE_Init: complete");
	return true;
}
...
RE_BeginFrame( float camera_separation )
{
	...
	if (stereo_enabled)
	{
		...
		else if (camera_separation > 0.0f)
		{
			next_eye = 1;
		}
		else
		{
			/* A zero depth scale still needs two identical packed eyes. */
			next_eye = (ps3_stereo_eye == 0) ? 1 : 0;
		}

		/*
		 * SCR_UpdateScreen calls BeginFrame for each eye but EndFrame only
		 * once, after both passes.  At the start of the right-eye pass the
		 * software buffer still contains the completed left-eye image.
		 */
		if (next_eye == 1 && stereo_left_frame && vid_buffer)
		{
			memcpy(stereo_left_frame, vid_buffer,
				vid_buffer_height * vid_buffer_width * sizeof(pixel_t));
		}

		ps3_stereo_eye = next_eye;
	}
	else
	{
		ps3_stereo_eye = -1;
...
{
	/* HDMI frame-packed stereo must remain synchronized as one eye pair. */
	if (r_vsync->value ||
		Cvar_VariableValue("ps3_stereo_enable") > 0.0f)
	{
		return true;
...
RE_ShutdownRenderer(void)
{
	...
	{
		free(stereo_left_frame);
	}
	stereo_left_frame = NULL;
	ps3_stereo_eye = -1;
...
{
	int vmin, vmax;
	qboolean stereo_left =
		(Cvar_VariableValue("ps3_stereo_enable") > 0.0f && ps3_stereo_eye == 0);
	qboolean stereo_frame =
		(Cvar_VariableValue("ps3_stereo_enable") > 0.0f && ps3_stereo_eye == 1);

	/* Keep the left eye without presenting or swapping the software buffer.
	 * The second pass redraws this buffer, then presents both eyes together. */
	if (stereo_left)
	{
		if (stereo_left_frame && vid_buffer)
		{
			memcpy(stereo_left_frame, vid_buffer,
				vid_buffer_height * vid_buffer_width * sizeof(pixel_t));
		}
	...
	if (!palette_changed && !stereo_frame)
	{
...
SWimp_SetMode(int *pwidth, int *pheight, int mode, int fullscreen )
{
	...
	PS3_BOOT_TRACE("SWimp_SetMode: before GLimp_InitGraphics");
	if (!ri.GLimp_InitGraphics(fullscreen, pwidth, pheight))
	{
		// failed to set a valid mode in windowed mode
		return rserr_invalid_mode;
	}
	PS3_BOOT_TRACE("SWimp_SetMode: complete");
...
SWimp_CreateRender(int width, int height)
{
	...
	stereo_left_frame = malloc(height * width * sizeof(pixel_t));
	if (!stereo_left_frame)
	{
		ri.Sys_Error(ERR_FATAL, "%s: Can't allocate stereo eye buffer.",
			__func__);
		return;
	}
	memset(stereo_left_frame, 0, height * width * sizeof(pixel_t));
