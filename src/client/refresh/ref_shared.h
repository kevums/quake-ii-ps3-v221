...
#include "../vid/header/ref.h"
  ...
  /* The PS3 PPU main-thread stack is too small for renderer VLAs such as the
   * maximum-particle vertex/color batches. Keep these transient arrays on the
   * heap; callers already pair every declaration with YQ2_VLAFREE(). */
  #define YQ2_VLA(TYPE, VARNAME, NUMELEMS) \
	TYPE * VARNAME = (TYPE *)malloc(sizeof(TYPE) * (NUMELEMS))
  #define YQ2_VLAFREE(VARNAME) \
