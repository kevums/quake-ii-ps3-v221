...
#ifdef UNICORE
extern game_import_t* GetGameAPI(game_import_t *import);
#endif
/*
...
SV_InitGameProgs(void)
{
	...
	PS3_BOOT_TRACE("SV_InitGameProgs: entered");
...
#ifdef UNICORE
	PS3_BOOT_TRACE("SV_InitGameProgs: before GetGameAPI");
	ge = (game_export_t*)GetGameAPI(&import);

	if (!ge)
	{
		Com_Error(ERR_DROP, "failed to load game API");
	}
#else
	...
	PS3_BOOT_TRACE("SV_InitGameProgs: before game Init");
	ge->Init();
	PS3_BOOT_TRACE("SV_InitGameProgs: game Init complete");
