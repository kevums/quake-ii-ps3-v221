...
/* Save copies run synchronously on the main thread. Keeping this 64 KiB
 * transfer buffer in static storage avoids overflowing the GameOS PPU stack. */
static byte save_copy_buffer[65536];
...
CopyFile(char *src, char *dst)
{
	...
	while (1)
	{
		l = fread(save_copy_buffer, 1, sizeof(save_copy_buffer), f1);
		...
		fwrite(save_copy_buffer, 1, l, f2);
	}
...
SV_CopySaveGame(char *src, char *dst)
{
	...
	PS3_BOOT_TRACE("SV_CopySaveGame: entered");

	Com_DPrintf("SV_CopySaveGame(%s, %s)\n", src, dst);

	SV_WipeSavegame(dst);
	PS3_BOOT_TRACE("SV_CopySaveGame: destination cleared");
	...
	FS_CreatePath(name2);
	CopyFile(name, name2);
	PS3_BOOT_TRACE("SV_CopySaveGame: server state copied");
	...
	CopyFile(name, name2);
	PS3_BOOT_TRACE("SV_CopySaveGame: game state copied");
	...
	Sys_FindClose();
	PS3_BOOT_TRACE("SV_CopySaveGame: complete");
}
...
SV_WriteServerFile(qboolean autosave)
{
	...
	PS3_BOOT_TRACE("SV_WriteServerFile: entered");
	...
	fclose(f);
	PS3_BOOT_TRACE("SV_WriteServerFile: server state written");
	...
	PS3_BOOT_TRACE("SV_WriteServerFile: before game state write");
	ge->WriteGame("game.ssv", autosave);
	PS3_BOOT_TRACE("SV_WriteServerFile: game state written");

	Sys_SetWorkDir(workdir);
	PS3_BOOT_TRACE("SV_WriteServerFile: complete");
}
