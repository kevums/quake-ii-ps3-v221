		...
		qboolean attractloop, qboolean loadgame, qboolean isautosave)
{
	...
	PS3_BOOT_TRACE("SV_SpawnServer: entered");
	...
	else
	{
		...
		PS3_BOOT_TRACE("SV_SpawnServer: before server CM_LoadMap");
		sv.models[1] = CM_LoadMap(sv.configstrings[CS_MODELS + 1],
				false, &checksum);
		PS3_BOOT_TRACE("SV_SpawnServer: server CM_LoadMap complete");
	}
	...
	SV_ClearWorld();
	PS3_BOOT_TRACE("SV_SpawnServer: world links initialized");
	...
	PS3_BOOT_TRACE("SV_SpawnServer: before SpawnEntities");
	ge->SpawnEntities(sv.name, CM_EntityString(), spawnpoint);
	PS3_BOOT_TRACE("SV_SpawnServer: SpawnEntities complete");

	/* run two frames to allow everything to settle */
	ge->RunFrame();
	ge->RunFrame();
	PS3_BOOT_TRACE("SV_SpawnServer: settle frames complete");
	...
	SV_CreateBaseline();
	PS3_BOOT_TRACE("SV_SpawnServer: baseline complete");

	/* check for a savegame */
	SV_CheckForSavegame(isautosave);
	PS3_BOOT_TRACE("SV_SpawnServer: savegame check complete");

	/* set serverinfo variable */
	Cvar_FullSet("mapname", sv.name, CVAR_SERVERINFO | CVAR_NOSET);

	Com_Printf("------------------------------------\n\n");
	PS3_BOOT_TRACE("SV_SpawnServer: complete");
}
...
SV_InitGame(void)
{
	...
	PS3_BOOT_TRACE("SV_InitGame: entered");
	...
	PS3_BOOT_TRACE("SV_InitGame: client state allocated");
	...
	/* init game */
	PS3_BOOT_TRACE("SV_InitGame: before game progs");
	SV_InitGameProgs();
	PS3_BOOT_TRACE("SV_InitGame: game progs complete");
	...
	PS3_BOOT_TRACE("SV_InitGame: complete");
}
...
SV_Map(qboolean attractloop, char *levelstring, qboolean loadgame, qboolean isautosave)
{
	...
	{
		PS3_BOOT_TRACE("SV_Map: before SV_InitGame");
		SV_InitGame(); /* the game is just starting */
		PS3_BOOT_TRACE("SV_Map: SV_InitGame complete");
	}
	...
	else
	{
		...
		PS3_BOOT_TRACE("SV_Map: before gameplay spawn");
		SV_SpawnServer(level, spawnpoint, ss_game, attractloop, loadgame, isautosave);
		PS3_BOOT_TRACE("SV_Map: gameplay spawn complete");
		Cbuf_CopyToDefer();
	}
